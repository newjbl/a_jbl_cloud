...
func connect_to_server(poolmax=10) -> void:
	print("[tcp_transf_class]->connect_to_server:taskid=%s"%[taskid])
...
func query_files(filedic:Dictionary) -> void:
	#print('[tcp_transf_class]->query_files.')
	connect_to_server()
	var r = login_do()
	if not r:
...
func query_files_thread(filedic:Dictionary) -> void:
	#print('[tcp_transf_class]->query_files_thread.')
	...
	while upload_running and loop_cnt <= 3:
		...
		if r[0] == REQ_HEADER:
			...
			if rt_status == 'OK':
				...
				print('[tcp_transf_class]->query_files_thread:rt_status is %s, will try for %s time'%[rt_status, loop_cnt])
		else:
			print('[tcp_transf_class]->query_files_thread:failed, will try for %s time'%[loop_cnt])	
	query_report_result('', 'FAILED')
	print('[tcp_transf_class]->query_files_thread finish.')

func query_report_result(msg:String, rt:String) -> void:
	upload_running = false
	disconnect_to_server()
	print('[tcp_transf_class]->query_report_result. %s, %s'%[msg, rt])
	emit_signal('report_result', 'tcp_transf_class', taskid, 'query', msg, rt)
	
func rec_a_datablock(timeout=3) -> Array:
	#print('[tcp_transf_class]->rec_a_datablock')
	if _socket == null:
		print('[tcp_transf_class]->rec_a_datablock:_socket is null')
		return ['', 'error-1']
	var stime:int = Time.get_ticks_msec()
	#while _socket_alive and _socket.get_status() == StreamPeerTCP.STATUS_CONNECTED and _socket.get_available_bytes() < 10:
	while _socket_alive and _socket.get_available_bytes() < 10:
		...
		if header == REQ_HEADER:
			...
			if len_int > 0:
				...
				if data2[0] == Error.OK and data2[1].size() > 0:
					...
					print('[tcp_transf_class]->rec_a_datablock:error13')
					return ['', 'error13']
			print('[tcp_transf_class]->rec_a_datablock:error14')
			return ['', 'error14']
			...
			if len_int > 0:
				...
				if data2[0] == Error.OK and data2[1].size() > 0:
					...
					print('[tcp_transf_class]->rec_a_datablock:error23')
					return ['', 'error23']
			print('[tcp_transf_class]->rec_a_datablock:error24')
			return ['', 'error24']
		else:
			print('[tcp_transf_class]->rec_a_datablock:error2')
			return ['', 'error2']
	else:
		print('[tcp_transf_class]->rec_a_datablock:error1')
		return ['', 'error1']
...
func login_do(loopmax=3) -> bool:
	...
	if _socket.get_status() != StreamPeerTCP.STATUS_CONNECTED:
		print("[tcp_transf_class]->login_do failed due to _socket status is %s"%[_socket.get_status()])
		return false
	var loop_cnt = 0
	while loop_cnt < loopmax:
		...
		if r[0] == REQ_HEADER:
			...
			if rt_status == 'OK':
				...
				return true
			print("[tcp_transf_class]->login_do:login failed"%[r[1].get('message', 'unknown error')])
		loop_cnt += 1
	print("[tcp_transf_class]->login_do:login failed")
	return false
		
func disconnect_to_server() -> void:
	print("[tcp_transf_class]->disconnect_to_server:taskid=%s"%[taskid])
	if _socket == null:
		print("[tcp_transf_class]->disconnect_to_server. _socket is null")
		return
	download_running = false
	upload_running = false
	dl_tmpfilepath = r''
	...
	while upload_running and loop_cnt <= 3:
		...
		if r1[0] == REQ_HEADER:
			...
			if rt_status1 == 'OK':
				...
				var file_size = FileAccess.get_size(filepath)
				var dynamic_timeout = max(3.0, file_size / 1024.0 / 1024.0 * 10.0)
				var r2 = rec_a_datablock(dynamic_timeout)
				if r2[0] == REQ_HEADER:
					var rt_status2 = r2[1].get('status', '')
					if rt_status2 == 'FINISH':
						print('[tcp_transf_class]->upload_a_file_thread:%s upload finish, get response from server'%[filepath])
						upload_report_result('FINISH')
						return
					else:
						print('[tcp_transf_class]->upload_a_file_thread:%s upload failed2:%s'%[filepath, rt_status2])
			elif rt_status1 == 'ERROR2':
				print('[tcp_transf_class]->upload_a_file_thread:%s upload finish, already on server'%[filepath])
				upload_report_result('ERROR2')
				return
			else:
				print('[tcp_transf_class]->upload_a_file_thread:%s upload failed1:%s'%[filepath, rt_status1])
	upload_report_result('FAILED')
	return
...
func upload_data(filepath, offset) -> void:
	...
	print("[tcp_transf_class]->upload_data finish:%s  %s"%[filepath, offset])

func upload_report_result(rt:String) -> void:
		print('[tcp_transf_class]->upload_report_result:%s'%[rt])
		upload_running = false
		disconnect_to_server()
...
func download_a_file(filepath:String) -> void:
	print("[tcp_transf_class][%s]->download_a_file:%s"%[taskid, filepath])
	if not filepath.ends_with('/files.txt'):
				...
				if rr[0]:
					...
					while true:
						...
						if rrr[0] == DO_HEADER:
							...
							idx = wr['idx']
							if idx % 50 == 0:
								emit_signal("report_result", 'tcp_transf_class', taskid, 'download', '%s;%s'%[current_size, sv_file_size], 'PROCESS')
							if current_size >= sv_file_size:
								download_running = false
								print('[tcp_transf_class]->write_a_file_thread:stop download due to current_size >= file_size!')
								break
					emit_signal("report_result", 'tcp_transf_class', taskid, 'download', '%s;%s'%[current_size, sv_file_size], 'PROCESS')
					f.close()
					...
					if download_file_md5 == md5_check:
						...
						print('[tcp_transf_class]->write_a_file_thread:md5 is ok, download finish!!')
						download_report_result('FINISH')
						return
					else:
						print('[tcp_transf_class]->write_a_file_thread:md5 is error, will retry %s time'%loop_cnt)
				else:
					print('[tcp_transf_class]->write_a_file_thread:prepare file failed, will retry %s time'%loop_cnt)						
			elif rt_status == 'ERROR7':
				download_report_result('ERROR7')
				return
			else:
				print("[tcp_transf_class]->download_a_file_thread:rt_status is %s, will try for %s time"%[rt_status, loop_cnt])
		else:
			print("[tcp_transf_class]->download_a_file_thread:get req_header failed, will try for %s time"%loop_cnt)
	print("[tcp_transf_class]->download_a_file_thread finish:%s"%[filepath])
	
func download_file_prepare_name_overwrite(filepath:String) -> bool:
	if FileAccess.file_exists(filepath):
		var err = DirAccess.remove_absolute(filepath)
		if err != Error.OK:
			print('[tcp_transf_class]->download_file_prepare_name_overwrite:remove file failed in download overwrite mode')
			return false
		return true
	return true
...
func download_report_result(rt:String) -> void:
	download_running = false
	disconnect_to_server()
	print('[tcp_transf_class]->download_report_result:download %s!!'%rt)
...
func write_a_data_block(f:FileAccess, data_block:PackedByteArray, preidx:int) -> Dictionary:
	...
	if idxint != 0 and idxint - preidx != 1:
		print("write_a_data_block: idx not continue: %s, %s"%[idxint, preidx])
	...
	if idxint % 100 == 0:
		print('[tcp_transf_class]->write_a_data_block:idx process:%s'%idxint)
	if f:
		f.seek_end()
...
func request_a_message(req_dic:Dictionary):
	#print("[tcp_transf_class]->request_a_message:|GD>SV|RQ:%s is %s"%[_socket.get_status(), req_dic])
	if _socket.get_status() == StreamPeerTCP.STATUS_CONNECTED:
...
func _destory() -> void:
	print("[tcp_transf_class][%s]->_destory start"%taskid)
	disconnect_to_server()
	_socket = null
	if download_thread:
	...
	if crc32_class:
		crc32_class.free()
	queue_free()	
	print("[tcp_transf_class][%s]->_destory finish"%taskid)
