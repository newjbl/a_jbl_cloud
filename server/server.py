...
def get_socket_timeout(filesize):
    return max(int(10 * filesize / 1024 / 1024), UE_TIMEOUT)
...
def handle_ue_upload_details(upload_socket:socket.socket, client_addr:tuple, meta_json, upload_text):
    ...
    upload_socket.settimeout(new_timeout)
...
def handle_ue_download_details(download_socket:socket.socket, client_addr:tuple, meta_json, download_text):
    ...
    new_timeout = get_socket_timeout(sv_file_size)
    print("[%s]ue(%s) request download new timeout: %s, %s" % (datetime.now(), client_addr, new_timeout, filepath))
    download_socket.settimeout(new_timeout)
